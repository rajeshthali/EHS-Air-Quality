define([ 'angular', './services-module' ], function(angular, sampleModule) {
	'use strict';
	sampleModule.factory('WasteManagementService', [ '$http', '$rootScope', 'AuthService', 'Config', function($http, $rootScope, AuthService, Config) {
		return {
			getWasteConsumptionValues : function(floor, interval, cb) {
				var thisObject = this;
				if (!$rootScope.token) {
					AuthService.getTocken(function(token) {
						thisObject.getWasteConsumption($rootScope.token, floor, interval, cb);
					});
				} else {
					thisObject.getWasteConsumption($rootScope.token, floor, interval, cb);
				}
			},
			getWasteConsumption : function(token, floor, interval, cb) {
				$http({
					method : 'GET',
